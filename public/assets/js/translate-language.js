...
(function(){
    ...
    function get_lang_href(lang) {
        ...
        if (url_structure == 'sub_directory') {
            ...
            var gt_request_uri2 = (document.currentScript.getAttribute('data-gt-orig-url') || location.pathname) + location.search + location.hash;
            var domain = document.currentScript.getAttribute('data-gt-orig-domain') || location.hostname;
            if (typeof custom_domains == 'object' && custom_domains[lang]) {
                href = (lang == default_language) && location.protocol + '//' + domain + gt_request_uri2 || location.protocol + '//' + custom_domains[lang] + gt_request_uri2;
            } else {
                href = (lang == default_language) && location.protocol + '//' + domain + gt_request_uri2 || location.protocol + '//' + lang + '.' + domain.replace(/^www\./, '') + gt_request_uri2;
            }
        }
    ...
    widget_css += '.gt_float_switcher{font-size:20px;border-radius:2px;color:#555;display:inline-block;line-height:20px;overflow:hidden;transition:all .5s cubic-bezier(0.4, 0, 1, 1)}';
    ...
    var gt_current_div = '<div class="gt-selected"><div class="gt-current-lang"><img src="' + get_flag_src(current_lang) + '" alt="' + current_lang + '"> <span class="gt-lang-code">' + (lang_array[current_lang] || current_lang) + '</span> <span class="gt_float_switcher-arrow"></span></div></div>';
    ...
    languages.forEach(function(lang) {
        ...
        if (current_lang == lang) el_a.classList.add('gt-current');
        ...
        el_a.innerHTML += ' ' + (lang_array[lang] || lang);
    ...
    function gt_update_float_language(el) {
        ...
        setTimeout(function(){
            var current = el.parentNode.querySelector('a.gt-current');
            if (current) current.classList.remove('gt-current');
            el.classList.add('gt-current');
        }, 400);

        el.parentNode.parentNode.querySelector('.gt-selected img').setAttribute('src', img_src);
        el.parentNode.parentNode.querySelector('.gt-selected span.gt-lang-code').innerText = lang_array[lang] || lang;
        gt_hide_float_switcher();
    }

    setTimeout(function(){
        document.querySelectorAll(u_class + ' .gt_float_switcher').forEach(function(e){ e.style.opacity = 1; });
    }, 20);
    ...
    document.querySelectorAll(u_class + ' .gt_float_switcher').forEach(function(wrapper){
        wrapper.addEventListener('mouseenter', function(){
            wrapper.querySelectorAll('.gt_options img:not([src])').forEach(function(img){
                img.setAttribute('src', img.getAttribute('data-gt-lazy-src'));
            });
            gt_show_float_switcher(wrapper);
        });
